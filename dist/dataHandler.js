...
async function getTodos() {
    try {
        const data = await fetcher('/getTodos', { method: 'GET' });
...
function resetTodosInput() {
    if (!todoUserInput)
        throw new Error('No user input');
    todoUserInput.innerHTML = '';
}
function deleteTodo(todo, listItemID, deleteID) {
    ...
    deleteButton.addEventListener('click', async () => {
        ...
        if (!listItem)
            throw new Error('No list item');
        console.log(`${todo.title} deleted`);
        listItem.remove();
    });
}
function editTodo(todo, todoID, editID) {
    ...
    editButton.addEventListener('click', async () => {
        ...
        console.log(options);
        const data = await fetcher(`/${todo._id}`, options);
...
function buildTemplate(todo, ids) {
...
form.addEventListener('submit', async (event) => {
    ...
    try {
        const todo = await fetcher('/', fetchOptions);
        let ids = buildIds(todo);
